    ...
    <Box className="p-4 rounded-md bg-gray-3">
      <Flex direction="column" gap="2">
        ...
        {showInterest1 && (
          <Text>
            <strong>1차 이자:</strong>{" "}
            {Math.floor(totalInterest1).toLocaleString()}원{" "}
            <span className="text-sm text-gray-11">
              ({formatDate(bond.interest_1_start_date)} ~{" "}
              {formatDate(bond.interest_1_end_date)})
            </span>
          </Text>
        )}
        ...
        {showInterest2 && (
          <Text>
            <strong>2차 이자:</strong>{" "}
            {Math.floor(totalInterest2).toLocaleString()}원{" "}
            <span className="text-sm text-gray-11">
              ({formatDate(bond.interest_2_start_date)} ~{" "}
              {formatDate(bond.interest_2_end_date)})
            </span>
          </Text>
        )}
