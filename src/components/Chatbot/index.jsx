...
const scenarios = {
  ...
  construction: [
    ...
    },
    {
      ...
      type: QUESTION_TYPES.TRANSACTIONS_TABLE,
      question:
        "공사 기간과 대금을 입력해 주세요.\n각 공사별로 시작일, 종료일, 금액, 지급약정일을 입력해 주세요.",
    },
    {
      id: "confirm",
      type: QUESTION_TYPES.TEXT,
...
function TransactionsTable({ transactions, setTransactions, type }) {
  ...
  const handleAdd = () => {
    ...
    if (type === "construction") {
      if (!startDate || !endDate || !amount || !dueDate) {
        alert("모든 항목을 입력해주세요.");
        return;
      }
    } else {
      if (!transactionDate || !amount || !dueDate) {
        alert("모든 항목을 입력해주세요.");
        return;
      }
    }
