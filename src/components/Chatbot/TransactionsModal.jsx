...
export default function TransactionsModal({ type, transactions, setTransactions, onClose }) {
	...
	const handleAddOrUpdate = () => {
		...
		const newTransaction = {
			id:
				editIndex !== null && transactions[editIndex]
		...
		if (editIndex !== null) {
			const updatedTransactions = transactions.map((item, index) =>
				index === editIndex ? newTransaction : item
			);
	...
	return (
		<Dialog.Root open>
			...
			<Dialog.Content className="fixed top-1/2 left-1/2 max-w-3xl w-full max-h-[80vh] overflow-y-auto transform -translate-x-1/2 -translate-y-1/2 bg-gray-2 p-6 rounded-lg shadow-lg border border-gray-6 min-h-[600px]">
				<Dialog.Title className="text-xl font-bold mb-4 text-gray-12">
					거래 내역 입력
				</Dialog.Title>
				...
				<Box className="mb-4">
					...
					<Button variant="soft" color="blue" className="w-full mt-4" onClick={handleAddOrUpdate}>
						{editIndex !== null ? "수정하기" : (<><PlusIcon className="mr-1" />추가하기</>)}
					</Button>
				</Box>
							...
							{type === "construction" ? (
								<>
									<Table.ColumnHeaderCell className="w-1/4 text-gray-12">
										<button className="flex items-center justify-center gap-1 w-full" onClick={() => handleSort("startDate")}>
											시작일 {renderSortIcon("startDate")}
										</button>
									</Table.ColumnHeaderCell>
									<Table.ColumnHeaderCell className="w-1/4 text-gray-12">
										<button className="flex items-center justify-center gap-1 w-full" onClick={() => handleSort("endDate")}>
											종료일 {renderSortIcon("endDate")}
										</button>
									</Table.ColumnHeaderCell>
									<Table.ColumnHeaderCell className="w-1/4 text-gray-12">
										<button className="flex items-center justify-center gap-1 w-full" onClick={() => handleSort("amount")}>
											금액 {renderSortIcon("amount")}
										</button>
									</Table.ColumnHeaderCell>
									<Table.ColumnHeaderCell className="w-1/4 text-gray-12">
										<button className="flex items-center justify-center gap-1 w-full" onClick={() => handleSort("dueDate")}>
								...
								<>
									<Table.ColumnHeaderCell className="w-1/3 text-gray-12">
										<button className="flex items-center justify-center gap-1 w-full" onClick={() => handleSort("transactionDate")}>
							...
							<Table.Row key={transaction.id}>
								{type === "construction" ? (
									<>
										...
										<Table.Cell className="text-gray-12 text-center">{transaction.amount.toLocaleString()}원</Table.Cell>
