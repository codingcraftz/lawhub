  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      if (groupIds.length > 0) {
        ...
        // 그룹 사건 데이터 가공
        formattedGroupAssignments = (groupAssignments ?? []).map(
          (assignment) => ({
            ...assignment,
            type: "그룹",
            name:
              assignment.assignment_groups?.groups?.name || "알 수 없는 그룹",
          }),
        );
      }

      // ✅ 3) 개인 + 그룹 사건 합치기
      const formattedPersonalAssignments = (personalAssignments ?? []).map(
        (assignment) => ({
          ...assignment,
          type: "개인",
          name:
            assignment.assignment_clients?.users?.name || "알 수 없는 사용자",
        }),
      );

      const all = [
        ...formattedPersonalAssignments,
        ...formattedGroupAssignments,
      ];
      setAssignments(all);
