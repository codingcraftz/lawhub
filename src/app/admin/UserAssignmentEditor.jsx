...
export default function UserAssignmentEditor({ user, assignments }) {
	...
	const fetchUserAssignments = async () => {
		...
		const { data, error } = await supabase
			...
			.select("id, assignment_id, assignments!inner(description, status, created_at)")
			.eq("user_id", user.id);
	...
	return (
		<Box>
			...
			{selectedEntity && (
				<Card p="3" className="mb-4 border border-gray-6">
					<Text size="3" weight="bold" className="mb-2">
						[{selectedEntity.type === "group" ? "단체" : "개인"}]선택된 대상: {selectedEntity.name}
					</Text>

					{entityAssignments.length === 0 ? (
						<h2 className="text-gray-11 py-2">해당 고객의 등록된 의뢰가 없습니다.</h2>
					) : (
					...
					{userAssignments.map((ua) => (
						...
						>
							<Box className="flex gap-2 items-center">
								...
								<Text size="2" color="gray">
									{new Date(ua.assignments.created_at).toLocaleDateString("ko-KR")}
								</Text>
							</Box>
							<Button
								variant="ghost"
