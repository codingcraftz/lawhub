...
export default function SentTasks({ user, preview = false }) {
  ...
  const fetchTasks = async () => {
    ...
    try {
      // 전체 개수 조회를 위한 쿼리
      let countQuery = supabase.from('tasks').select('id', { count: 'exact' }).eq('created_by', user.id); // 보낸 업무만 조회
      ...
      let query = supabase
        ...
        .eq('created_by', user.id); // 보낸 업무만 조회
