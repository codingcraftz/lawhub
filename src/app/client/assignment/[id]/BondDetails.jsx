...
const BondDetails = ({ assignmentId, user }) => {
  ...
  const totalAmount =
    principal + totalInterest1 + totalInterest2 + totalExpenses;
  ...
  return (
    <section className="mb-6 p-4 rounded shadow-md shadow-gray-7 bg-gray-2 text-gray-12">
      <Flex justify="between" align="center" className="mb-3">
        ...
        {isAdmin && (
          <Button onClick={() => setIsFormOpen(true)}>등록/수정</Button>
        )}
      </Flex>
        ...
        <Text as="p" className="text-lg font-bold text-blue-11">
          ...
          {isAdmin && (
            <Text as="p" className="text-base font-normal text-gray-11">
              원금+비용:{" "}
              {Math.floor(principal + totalExpenses).toLocaleString()}
            </Text>
          )}
        </Text>
              ...
              <ul className="list-disc ml-5 space-y-1">
                {bond.expenses.map((expense, idx) => (
                  <li key={idx} className="text-sm">
                    {expense.item}: {parseInt(expense.amount).toLocaleString()}
                    원
                  </li>
                ))}
              </ul>
