// src/app/case-mangement/_components/DebtorSelectionModalContent

import React, { useState, useEffect } from "react";
import { useForm } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import * as yup from "yup";
import { supabase } from "@/utils/supabase";
import {
  Box,
  Flex,
  Text,
  Button,
  Checkbox,
  Tooltip,
  Theme,
} from "@radix-ui/themes";
import * as Dialog from "@radix-ui/react-dialog";
import { Cross2Icon } from "@radix-ui/react-icons";
import DebtorForm from "./DebtorForm";

const schema = yup.object().shape({
  name: yup.string().required("이름은 필수입니다"),
  registration_number: yup
    .string()
    .matches(/^\d{13}$/, "주민등록번호는 13자리 숫자여야 합니다")
    .required("주민등록번호는 필수입니다"),
  phone_number: yup
    .string()
    .matches(/^\d{10,11}$/, "전화번호는 10~11자리의 숫자여야 합니다")
    .required("전화번호는 필수입니다"),
  address: yup.string().required("주소는 필수입니다"),
});

const DebtorSelector = ({
  open,
  onOpenChange,
  selectedDebtors = [],
  setSelectedDebtors,
  onClose,
}) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [allDebtors, setAllDebtors] = useState([]);
  const [filteredDebtors, setFilteredDebtors] = useState([]);
  const [localSelectedDebtors, setLocalSelectedDebtors] = useState([
    ...selectedDebtors,
  ]);
  const [isAddingDebtor, setIsAddingDebtor] = useState(false);

  // React Hook Form setup
  const {
    register,
    handleSubmit,
    formState: { errors, isValid },
  } = useForm({
    resolver: yupResolver(schema),
    mode: "onChange",
  });

  useEffect(() => {
    const fetchDebtors = async () => {
      const { data, error } = await supabase.from("debtors").select("*");
      if (error) {
        console.error("Error fetching debtors:", error);
      } else {
        setAllDebtors(data || []);
        setFilteredDebtors(data || []);
      }
    };
    fetchDebtors();
  }, []);

  useEffect(() => {
    if (searchTerm === "") {
      setFilteredDebtors(allDebtors);
    } else {
      const lowerCaseSearchTerm = searchTerm.toLowerCase();
      const filtered = allDebtors.filter((debtor) =>
        debtor.name.toLowerCase().includes(lowerCaseSearchTerm),
      );
      setFilteredDebtors(filtered);
    }
  }, [searchTerm, allDebtors]);

  const formatPhoneNumber = (phoneNumber) => {
    if (!phoneNumber) return "없음";
    return phoneNumber.replace(/(\d{3})(\d{4})(\d{4})/, "$1-$2-$3");
  };

  const formatRegistrationNumber = (registrationNumber) => {
    if (!registrationNumber) return "없음";
    return registrationNumber.replace(/(\d{6})(\d{7})/, "$1-$2");
  };

  const handleToggleDebtor = (debtor) => {
    if (localSelectedDebtors.find((o) => o.id === debtor.id)) {
      setLocalSelectedDebtors((prev) => prev.filter((o) => o.id !== debtor.id));
    } else {
      setLocalSelectedDebtors((prev) => [...prev, debtor]);
    }
  };

  const handleSaveSelection = () => {
    setSelectedDebtors(localSelectedDebtors);
    onClose();
  };

  const handleAddNewDebtor = async (data) => {
    try {
      const { data: newDebtorData, error } = await supabase
        .from("debtors")
        .insert([data])
        .select("*");

      if (error) {
        console.error("Error adding debtor:", error);
        alert("상대방 추가 중 오류가 발생했습니다.");
        return;
      }

      // 새 상대방 추가 및 선택
      setAllDebtors((prev) => [...prev, newDebtorData[0]]);
      setLocalSelectedDebtors((prev) => [...prev, newDebtorData[0]]);

      // 추가 후 상대방 선택 화면 유지
      setIsAddingDebtor(false); // 상대방 추가 폼을 닫고 리스트로 복귀
    } catch (error) {
      console.error("Error adding debtor:", error);
      alert("상대방 추가 중 오류가 발생했습니다.");
    }
  };

  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Theme>
          <Dialog.Overlay className="fixed inset-0 bg-black opacity-75 z-30" />
          <Dialog.Content className="fixed bg-gray-3 left-1/2 top-1/2 max-h-[85vh] min-w-[500px] max-w-[650px] -translate-x-1/2 -translate-y-1/2 rounded-md p-[25px] shadow focus:outline-none data-[state=open]:animate-contentShow z-40">
            <Dialog.Close>
              <Button
                variant="ghost"
                color="gray"
                style={{ position: "absolute", top: 8, right: 8 }}
              >
                <Cross2Icon width={25} height={25} />
              </Button>
            </Dialog.Close>
            <Dialog.Title>채무자 추가</Dialog.Title>
            {isAddingDebtor ? (
              <DebtorForm
                onSubmit={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  handleSubmit(handleAddNewDebtor)(e);
                }}
                register={register}
                errors={errors}
                open={isAddingDebtor}
                onOpenChange={setIsAddingDebtor}
              />
            ) : (
              <Box>
                <>
                  <input
                    type="text"
                    placeholder="검색"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    style={{
                      width: "100%",
                      padding: "1rem",
                      border: "1px solid var(--gray-6)",
                      borderRadius: "var(--radius-1)",
                    }}
                  />

                  <Box mt="3" style={{ maxHeight: "300px", overflowY: "auto" }}>
                    {filteredDebtors.map((debtor) => (
                      <Flex key={debtor.id} align="center" mt="2">
                        <Checkbox
                          checked={
                            !!localSelectedDebtors.find(
                              (o) => o.id === debtor.id,
                            )
                          }
                          onCheckedChange={() => handleToggleDebtor(debtor)}
                        />
                        <Tooltip
                          content={
                            <Box
                              style={{
                                padding: "0.5rem",
                                fontSize: "12px",
                                color: "var(--gray-12)",
                              }}
                            >
                              <p>
                                <strong>전화번호: </strong>
                                {formatPhoneNumber(debtor.phone_number)}
                              </p>
                              <p>
                                <strong>주소: </strong> {debtor.address}
                              </p>
                              <p>
                                <strong>주민등록번호: </strong>
                                {formatRegistrationNumber(
                                  debtor.registration_number,
                                )}
                              </p>
                            </Box>
                          }
                        >
                          <Text ml="2" style={{ cursor: "pointer" }}>
                            {debtor.name}
                          </Text>
                        </Tooltip>
                      </Flex>
                    ))}
                  </Box>
                  <Flex justify="between" mt="4" gap="2">
                    <Button onClick={() => setIsAddingDebtor(true)}>
                      상대방 추가
                    </Button>
                    <Flex className="gap-6">
                      <Button variant="outline" onClick={onClose}>
                        취소
                      </Button>
                      <Button onClick={handleSaveSelection}>선택 완료</Button>
                    </Flex>
                  </Flex>
                </>
              </Box>
            )}
          </Dialog.Content>
        </Theme>
      </Dialog.Portal>
    </Dialog.Root>
  );
};

export default DebtorSelector;
